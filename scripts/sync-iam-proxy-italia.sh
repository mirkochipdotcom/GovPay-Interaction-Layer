...
" 2>/dev/null)
  ...
  if [ -z "${CIE_OIDC_CONTACT_EMAIL:-}" ]; then
    echo "[sync-iam-proxy] ERROR: CIE_OIDC_CONTACT_EMAIL is empty. It is mandatory for metadata.federation_entity.contacts"
    exit 1
  fi
  ...
  # Inject CIE OIDC JWKS keys from metadata/cieoidc-keys/ (generated by metadata/setup-cie-oidc.sh)
  # Replaces __CIE_OIDC_JWK_*_YAML__ placeholders with the actual JWK fields in YAML format.
  CIEOIDC_KEYS_DIR="$REPO_ROOT/metadata/cieoidc-keys"
  JWK_FED="$CIEOIDC_KEYS_DIR/jwk-federation.json"
  JWK_SIG="$CIEOIDC_KEYS_DIR/jwk-core-sig.json"
  JWK_ENC="$CIEOIDC_KEYS_DIR/jwk-core-enc.json"

  if [ ! -f "$JWK_FED" ] || [ ! -f "$JWK_SIG" ] || [ ! -f "$JWK_ENC" ]; then
    echo "[sync-iam-proxy] ERROR: Chiavi CIE OIDC mancanti in $CIEOIDC_KEYS_DIR" >&2
    echo "[sync-iam-proxy]        Esegui prima: bash metadata/setup-cie-oidc.sh" >&2
    exit 1
  fi

  echo "[sync-iam-proxy] Injecting CIE OIDC JWKS from $CIEOIDC_KEYS_DIR..."
  python3 - \
      "$PROJECT_DST/conf/backends/cieoidc_backend.yaml" \
      "$JWK_FED" "$JWK_SIG" "$JWK_ENC" <<'PY'
import sys
import json
from pathlib import Path

yaml_path = Path(sys.argv[1])
paths = {"__CIE_OIDC_JWK_FEDERATION_YAML__": Path(sys.argv[2]),
         "__CIE_OIDC_JWK_CORE_SIG_YAML__":   Path(sys.argv[3]),
         "__CIE_OIDC_JWK_CORE_ENC_YAML__":   Path(sys.argv[4])}

# JWK field order: put well-known public fields first, private fields after
FIELD_ORDER = ["use", "alg", "kty", "kid", "e", "n", "d", "p", "q"]

def jwk_to_yaml_block(jwk: dict, indent: int) -> str:
    """Convert a JWK dict to YAML key: value lines at the given indentation."""
    lines = []
    keys_ordered = sorted(jwk.keys(), key=lambda k: (FIELD_ORDER.index(k) if k in FIELD_ORDER else 99, k))
    for i, key in enumerate(keys_ordered):
        val = json.dumps(jwk[key])  # quoted string or number
...
for placeholder, jwk_path in paths.items():
    ...
    # Find the line containing the placeholder to detect its indentation
    new_lines = []
    for line in content.splitlines():
        stripped = line.lstrip()
        if stripped.startswith(f"- {placeholder}") or stripped == f"- {placeholder}":
            # Determine indentation of the "- " prefix
...
PY
  ...
  echo "[sync-iam-proxy] CIE OIDC JWKS injection completed"
fi

# Install custom CIE OIDC federation endpoint handlers (resolve/fetch/trust_mark_status/list)
if [ -d "$REPO_ROOT/iam-proxy/cieoidc-endpoints" ]; then
  echo "[sync-iam-proxy] Installing custom CIE OIDC federation endpoint handlers..."
...
# Align CIE OIDC target issuer mapping with configured provider URL.
if [ -f "$PROJECT_DST/conf/microservices/target_based_routing.yaml" ] && is_true "$ENABLE_CIE_OIDC"; then
  ROUTING_FILE="$PROJECT_DST/conf/microservices/target_based_routing.yaml"
  CIE_OIDC_ISSUER="${CIE_OIDC_PROVIDER_URL:-http://cie-provider.example.org:8002/oidc/op/}"
  echo "[sync-iam-proxy] Ensuring CIE OIDC target mapping for issuer: $CIE_OIDC_ISSUER"

  # Remove previous CIE OIDC mapping rows, then enforce the configured one.
  sed -i '/"CieOidcRp"/d' "$ROUTING_FILE"
  sed -i "/\"wallet\": \"OpenID4VP\"/a\\    \"$CIE_OIDC_ISSUER\": \"CieOidcRp\"" "$ROUTING_FILE"
fi
...
if [ -f "$DISCO_TEMPLATE" ]; then
  ...
  export APP_VERSION
  envsubst '${APP_LOGO_SRC} ${APP_LOGO_TYPE} ${APP_ENTITY_NAME} ${APP_ENTITY_URL} ${FRONTOFFICE_PUBLIC_BASE_URL} ${SATOSA_UI_LEGAL_URL_IT} ${SATOSA_UI_PRIVACY_URL_IT} ${SATOSA_UI_ACCESSIBILITY_URL_IT} ${SATOSA_ORGANIZATION_URL_IT} ${SATOSA_ORGANIZATION_DISPLAY_NAME_IT} ${CIE_OIDC_PROVIDER_URL} ${APP_VERSION}' < "$DISCO_TEMPLATE" > "$DISCO_HTML"
